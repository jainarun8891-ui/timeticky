...
const content = HUB_PAGES_CUSTOM_CONTENT['/birthday-calculator'];
...
export default function BirthdayCalculatorPage() {
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <BirthdayCalculatorClient h1Title={content.h1} />
            ...
            {content.headings.slice(1).map((heading, idx) => (
              <div key={idx} className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/70 dark:border-slate-800 space-y-2">
                ...
                <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                  Zeller&apos;s congruence algorithm, quadriennial leap day adjustments, and lifetime heartbeats.
                </p>
              </div>
            ))}
          </div>
      ...
      <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
        <FaqAccordion items={content.faqs} title="Frequently Asked Questions: Birthday Chronology" />
      </div>
      <RelatedLinksHub />
    </div>
  );
